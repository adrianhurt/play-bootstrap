...
object HelpersSpec extends Specification {
  ...
  "@formGroup" should {
    ...
    def testFormGroup(args: (Symbol, Any)*)(fc: b3.B3FieldConstructor) =
      clean(b3.freeFormGroup(extraClasses = None, args)(innerArgs => Html("<content>"))(fc).body)
    ...
    "get the inner arguments for the content" in {
      val body = b3.freeFormGroup(extraClasses = None, Seq('_class -> "theClass", '_slashed -> "slashed", 'foo -> "foo"))(innerArgsMap => Html(innerArgsMap.toSeq.map(a => s"""${a._1.name}="${a._2.toString}"""").mkString("<content ", " ", ">")))(vfc).body
  ...
  "@free" should {
    "be rendered correctly" in {
      clean(b3.free('foo -> "fooValue")(Html("<content>"))(vfc).body) must be equalTo clean(b3.freeFormGroup(extraClasses = None, Seq('foo -> "fooValue"))(_ => Html("<content>"))(vfc).body)
    }
  }
