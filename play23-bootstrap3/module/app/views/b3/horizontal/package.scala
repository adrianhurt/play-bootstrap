...
package object horizontal {
  ...
  case class HorizontalFieldConstructor(colLabel: String, colInput: String) extends B3FieldConstructor {
    ...
    def apply(fieldInfo: B3FieldInfo, inputHtml: Html) = bsFieldConstructor(fieldInfo, inputHtml, colLabel, colOffset, colInput)
  ...
  def formCSRF(action: Call, colLabel: String, colInput: String, args: (Symbol, Any)*)(body: HorizontalFieldConstructor => Html)(implicit token: play.filters.csrf.CSRF.Token) = {
    val hfc = fieldConstructorSpecific(colLabel, colInput)
    views.html.b3.formCSRF(action, args: _*)(body(hfc))(hfc, token)
  }
