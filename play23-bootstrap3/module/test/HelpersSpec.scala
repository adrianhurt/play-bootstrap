...
object HelpersSpec extends Specification {
  ...
  "@checkbox" should {
    ...
    "allow setting a default value" in {
      val body = b3.checkbox(boolField, 'value -> true).body
  ...
  "@formGroup" should {
    ...
    def testFormGroup(args: (Symbol, Any)*)(fc: b3.B3FieldConstructor) =
      clean(b3.freeFormGroup(args)(innerArgs => Html("<content>"))(fc).body)
  ...
  "@free" should {
    "be rendered correctly" in {
      clean(b3.free('foo -> "fooValue")(Html("<content>"))(vfc).body) must be equalTo clean(b3.freeFormGroup(Seq('foo -> "fooValue"))(_ => Html("<content>"))(vfc).body)
    }
  }
  ...
  "@buttonType" should {
    ...
    def buttonTypeBody(args: (Symbol, Any)*) = b3.buttonType(sampleType, args: _*)(Html(sampleContent))(vfc).body
  ...
  def sampleButtonTypeBody(theType: String) = b3.buttonType(theType, sampleArgs: _*)(Html("content"))(vfc).body.trim

  "@submit" should {
    "be equivalent to buttonType with submit type" in {
      b3.submit(sampleArgs: _*)(Html("content"))(vfc).body.trim must be equalTo sampleButtonTypeBody("submit")
    }
  }
  "@reset" should {
    "be equivalent to buttonType with reset type" in {
      b3.reset(sampleArgs: _*)(Html("content"))(vfc).body.trim must be equalTo sampleButtonTypeBody("reset")
    }
  }
  "@button" should {
    "be equivalent to buttonType with button type" in {
      b3.button(sampleArgs: _*)(Html("content"))(vfc).body.trim must be equalTo sampleButtonTypeBody("button")
    }
  }
  ...
  "@multifield" should {
    ...
    def multifield(form: Form[(String, String)], globalArgs: Seq[(Symbol, Any)] = Seq(), fieldsArgs: Seq[(Symbol, Any)] = Seq())(fc: b3.B3FieldConstructor, lang: Lang) =
      clean(b3.multifield(form("foo"), form("bar"))(globalArgs, fieldsArgs)(cfc => Html(testInputsString))(fc, lang).body)
    def fooMultifield(globalArgs: (Symbol, Any)*) = multifield(fooForm, globalArgs)(vfc, lang)
    def fooMultifieldWithFielsArgs(fieldsArgs: (Symbol, Any)*) = multifield(fooForm, fieldsArgs = fieldsArgs)(vfc, lang)
    ...
    "behave as a horizontal field constructor" in {
      val body = multifield(fooForm, Seq('_label -> "theLabel"))(hfc, lang)
    ...
    "show label" in {
      multifield(fooForm, Seq('_label -> "fooLabel"))(vfc, lang) must contain("<label class=\"control-label\">fooLabel</label>")
      multifield(fooForm, Seq('_label -> "fooLabel"))(hfc, lang) must contain("<label class=\"control-label " + colLabel + "\">fooLabel</label>")
    }
    ...
    "allow showing constraints" in {
      multifield(fooForm, fieldsArgs = Seq('_showConstraints -> true))(vfc, lang) must contain("<span class=\"help-block\">" + Messages("constraint.required")(lang) + "</span>")
    }
    ...
    "render validation states" in {
      ...
      def testStatus(status: String, withIcon: Boolean, withFieldsArgs: Boolean, args: (Symbol, Any)*) = {
        val test = if (withFieldsArgs)
          clean(b3.multifield(fooForm("foo"))(globalArgs = if (withIcon) Seq('_hasFeedback -> true) else Seq(), fieldsArgs = args)(cfc => b3.text(fooForm("foo"), args: _*))(vfc, lang).body)
        else
          clean(b3.multifield(fooForm("foo"))(globalArgs = if (withIcon) (('_hasFeedback -> true) +: args) else args, fieldsArgs = Seq())(cfc => b3.text(fooForm("foo"), args: _*))(vfc, lang).body)
