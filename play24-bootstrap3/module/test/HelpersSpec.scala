...
object HelpersSpec extends Specification {
  ...
  "@checkbox" should {
    ...
    "ignore default and filled checked value if it has forced checked" in {
      val body1 = b3.checkbox(boolFieldFilled(false), '_default -> false, 'checked -> true).body
      body1 must contain("checked")
      body1 must contain("value=\"true\"")
      val body2 = b3.checkbox(boolFieldFilled(true), '_default -> true, 'checked -> false).body
      body2 must not contain ("checked")
      body2 must contain("value=\"true\"")
      val body3 = b3.checkbox(stringFieldFilled(""), 'value -> "bar", '_default -> false, 'checked -> true).body
      body3 must contain("checked")
      body3 must contain("value=\"bar\"")
      val body4 = b3.checkbox(stringFieldFilled("bar"), 'value -> "bar", '_default -> true, 'checked -> false).body
