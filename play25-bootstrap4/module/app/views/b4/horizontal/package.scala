...
package object horizontal {
  ...
  case class HorizontalFieldConstructor(colLabel: String, colInput: String) extends B4FieldConstructor {
    ...
    def apply(fieldInfo: B4FieldInfo, inputHtml: Html) = bsFieldConstructor(fieldInfo, inputHtml, colLabel, colOffset, colInput)
  ...
  def formCSRF(action: Call, colLabel: String, colInput: String, args: (Symbol, Any)*)(body: HorizontalFieldConstructor => Html)(implicit request: RequestHeader) = {
    val hfc = fieldConstructorSpecific(colLabel, colInput)
    views.html.b4.formCSRF(action, args: _*)(body(hfc))(hfc, request)
  }
